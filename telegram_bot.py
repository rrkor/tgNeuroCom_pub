...
async def start_program_handler(message: Message):
	try:
		...
		await message.answer("Запуск программы... Пожалуйста, подождите.",
							 reply_markup=get_main_menu(is_program_running=True))
		...
		logger.info(
			f"Программа запущена. Каналы: {len(channels)}, Прокси: {len(proxy_manager.working_proxies)}, Аккаунты: {len(accounts)}")
	except Exception as e:
...
async def stop_program_handler(message: Message):
	try:
		...
		await message.answer("Произошла ошибка при остановке программы. Попробуйте снова.",
							 reply_markup=get_main_menu())
...
async def show_accounts(message: types.Message):
	try:
			...
			if client:
				try:
					...
					accounts_info.append(
						f"{status_icon} ID: {os.path.basename(account['account_folder'])} | @{username}")
				except Exception as e:
		...
		await message.answer("Произошла ошибка при загрузке списка аккаунтов. Попробуйте снова.",
